...
from networks.sac_mlp import MLPActor, MLPSoftQNetwork
...
@dataclass
class Args:
    exp_name: str = "sac_fetch_baseline"
    run_name: str = "run"
    seed: int = 1
...
def train(args: Args, run_dir: str, trial: optuna.Trial | None = None) -> float:
    ...
    actor = MLPActor(envs).to(device)
    qf1 = MLPSoftQNetwork(envs).to(device)
    qf2 = MLPSoftQNetwork(envs).to(device)
    qf1_target = MLPSoftQNetwork(envs).to(device)
    qf2_target = MLPSoftQNetwork(envs).to(device)
